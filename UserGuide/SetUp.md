...
After this download, an executable file (*python-downloaded version-amd64.exe* for Windows) should appear in the Downloads folder of your computer. To finalize the installation of Python, the *.exe* file needs to be executed, i.e., you should double click on the file and follow the instructions prompted by the window:
...
For the purpose of this project, we chose to rely on Microsoft Visual Studio Code, a code editor that allows to write, edit, and run code on a local device. Therefore, in the following pages we will present the procedure to install, clone a repository, create a virtual environment, etc. within the specific set up of VS code. Nevertheless, other IDEs such as PyCharm or Jupyter Notebook can also be used to run this project, but their specificities will not be covered here. 

Similarly to the installation of Python, VS code can be installed on a local device by executing a *.exe* file downloaded on the following website: https://code.visualstudio.com/ 
...
To reproduce the results from this pledges' analysis locally, the Github repository we used (https://github.com/SEMICeu/semic_pledges) needs to be cloned to your local device. To reach this objective, the following steps should be followed: 
...
Create a new folder on your local device. This folder will be used to clone the GitHub repository
...
d. Sign into GitHub (or git) using your GitHub credentials (if asked)
